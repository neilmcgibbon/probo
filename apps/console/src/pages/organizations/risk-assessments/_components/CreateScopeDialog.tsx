  ...
  mutation CreateScopeDialogMutation(
    ...
    createRiskAssessmentScope(input: $input) {
      riskAssessmentScopeEdge @appendEdge(connections: $connections) {
        node {
          id name description
          nodes(first: 0) { __id edges { node { id } } }
          processes(first: 0) { __id edges { node { id } } }
          threats(first: 0) { __id edges { node { id } } }
        }
      }
    }
  }
...
export function CreateScopeDialog(props: { riskAssessmentId: string; connectionId: string }) {
  ...
  const onSubmit = (data: { name: string; description: string }) => {
    createScope({
      variables: {
        input: { riskAssessmentId: props.riskAssessmentId, name: data.name, description: data.description || null },
