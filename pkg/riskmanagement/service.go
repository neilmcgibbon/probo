...
type (
	...
	CreateRiskScenarioRequest struct {
		ThreatID    gid.GID
		RiskID      gid.GID
		Name        string
		Description *string
	...
	UpdateRiskScenarioRequest struct {
		ID          gid.GID
		Name        *string
		Description **string
	}
...
func (r *CreateRiskScenarioRequest) Validate() error {
	v := validator.New()
...
func (r *UpdateRiskScenarioRequest) Validate() error {
	v := validator.New()
	v.Check(r.ID, "id", validator.Required(), validator.GID(coredata.RiskScenarioEntityType))
	v.Check(r.Name, "name", validator.SafeTextNoNewLine(TitleMaxLength))
	v.Check(r.Description, "description", validator.SafeText(ContentMaxLength))
	return v.Error()
}
...
func (s *Service) UpdateScope(ctx context.Context, scope coredata.Scoper, req UpdateRiskAssessmentScopeRequest) (*coredata.RiskAssessmentScope, error) {
	...
	err := s.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			if req.Description != nil {
				raScope.Description = *req.Description
			}
...
func (s *Service) CreateThreat(ctx context.Context, scope coredata.Scoper, req CreateRiskAssessmentThreatRequest) (*coredata.RiskThreat, error) {
	if err := req.Validate(); err != nil {
		return nil, fmt.Errorf("invalid request: %w", err)
	}

	now := time.Now()
	threat := &coredata.RiskThreat{
...
func (s *Service) GetThreat(ctx context.Context, scope coredata.Scoper, id gid.GID) (*coredata.RiskThreat, error) {
	threat := &coredata.RiskThreat{}
	err := s.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
...
func (s *Service) UpdateThreat(ctx context.Context, scope coredata.Scoper, req UpdateRiskAssessmentThreatRequest) (*coredata.RiskThreat, error) {
	if err := req.Validate(); err != nil {
		return nil, fmt.Errorf("invalid request: %w", err)
	}
...
) (*page.Page[*coredata.RiskThreat, coredata.RiskAssessmentThreatOrderField], error) {
	var results coredata.RiskThreats
	err := s.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			if err := results.LoadByRiskAssessmentScopeID(ctx, conn, scope, scopeID, cursor); err != nil {
...
func (s *Service) CreateScenario(ctx context.Context, scope coredata.Scoper, req CreateRiskScenarioRequest) (*coredata.RiskScenario, error) {
	if err := req.Validate(); err != nil {
		return nil, fmt.Errorf("invalid request: %w", err)
	}

	now := time.Now()
	scenario := &coredata.RiskScenario{
		ID:          gid.New(scope.GetTenantID(), coredata.RiskScenarioEntityType),
	...
	err := s.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			if threat.OrganizationID != risk.OrganizationID {
				return validator.ValidationErrors{{
					Field:   "threat_id",
					Code:    validator.ErrorCodeCustom,
					Message: "threat and risk must belong to the same organization",
				}}
			}
			scenario.OrganizationID = risk.OrganizationID
...
func (s *Service) GetScenario(ctx context.Context, scope coredata.Scoper, id gid.GID) (*coredata.RiskScenario, error) {
	scenario := &coredata.RiskScenario{}
	err := s.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
...
func (s *Service) UpdateScenario(ctx context.Context, scope coredata.Scoper, req UpdateRiskScenarioRequest) (*coredata.RiskScenario, error) {
	if err := req.Validate(); err != nil {
		return nil, fmt.Errorf("invalid request: %w", err)
	}
...
func (s *Service) ListScenariosForRiskID(
	...
	scope coredata.Scoper,
	riskID gid.GID,
	cursor *page.Cursor[coredata.RiskScenarioOrderField],
) (*page.Page[*coredata.RiskScenario, coredata.RiskScenarioOrderField], error) {
	var results coredata.RiskScenarios
	err := s.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
